...
export function mountAtlas(opts: Opts): () => void {
  ...
  (mw.material as THREE.MeshBasicMaterial).color.setScalar(0.5);
  skyGroup.add(mw);
  {
    ...
    const pts = new THREE.Points(g, new THREE.PointsMaterial({ color: 0xdce4ff, size: 2.2, sizeAttenuation: false, transparent: true, opacity: 0.85, depthWrite: false }));
    skyGroup.add(pts);
  }
